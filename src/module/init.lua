    ...
    for _, number in ipairs({...}) do
        if isNumber(number) then
            ...
            if index > -9 and index < 9 then
                table.insert(returns, (string.gsub(string.format("%#." .. self.precision .. "f",  number / 10 ^ (index * 3)), "%.0*$", "")) .. scales[self.scale][index] )
            else
...
return setmetatable({precision = 3, scale = "SI", unit = ""}, {__call = toReadable})